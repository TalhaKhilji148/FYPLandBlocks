...
export default function ListProperty() {
  ...
  const [photos, setPhotos] = useState([]); // Store multiple photos as an array
...
const [file, setFile] = useState(null);
  ...
  const handleListProperty = async (e) => {
    ...
    try {
      ...
      formData.append("featured", state.featured);

      photos.forEach((photo, index) => {
        formData.append(`photos[${index}]`, photo);
      });

      const data = await request("/property", "POST", options, formData);
      ...
      >
        ...
        <form onSubmit={handleListProperty}>
          ...
          <div className="flex flex-row w-[100%] ml-[30%] ">
            ...
            />
            <label htmlFor="photo">
              Select Property pictures <AiOutlineFileImage />
            </label>
            {photos.length > 0 && (
              <ul>
